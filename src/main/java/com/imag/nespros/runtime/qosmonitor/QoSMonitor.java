...
public class QoSMonitor {
    ...
    public void addEPAgent(EPUnit epa) {
        EPNetwork.add(epa);
    }
    ...
    public ArrayList<EPUnit> getEPNetwork() {
        return EPNetwork;
    }

    public void setEPNetwork(ArrayList<EPUnit> EPNetwork) {
...
class MonitoringTask implements Runnable {
    ...
    public void run() {
        ...
        for (EPUnit epa : QoSMonitor.getInstance().getEPNetwork()) {
            ...
            int def = epa.getInputQueue().getNumberDefaults();
            int size = epa.getInputQueue().size();
